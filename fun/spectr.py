...
class SpectralAnalysis:
    ...
    def FnDopplerPeaks(self, spec_flat, peak_mode):
        ...
        default_reducer = 0.05
        while (not no_peaks.empty):
            # relax the parameters    
        ...
        def find_best_peaks(row:pd.Series, df_peaks:pd.DataFrame, n:int=2) -> Dict:
            """find the best n peaks in a row and using the structure of df_prop and df_peaks from find peaks algorithm"""
            ...
            # get the index of top n peaks
            best_peak_heights = pd.Series(row['peak_heights']).nlargest(n)
            best_peak_proms = pd.Series(row['prominences']).nlargest(n)
            best_idx = list(set(best_peak_heights.index) & set(best_peak_proms.index))

            # get the spectral positions based on index
            height_pos =  [df_peaks.iloc[int(row.name), :][f'peaks_{p}'] for p in best_peak_heights.dropna().index]
            prom_pos =  [df_peaks.iloc[int(row.name), :][f'peaks_{p}'] for p in best_peak_proms.dropna().index]
            best_pos = set(height_pos) & set(prom_pos)        
            
            # in case there are few peaks in intersection, get onl the peaks with prominences       
            if len(best_pos) < n:
                best_pos = set(prom_pos)
                best_idx = list(set(best_peak_proms.index))

            # get the properties of top n peaks
            # best_prop = pd.Series(dtype='float64')
            # for k, v in row.items():
            #     best_prop[k] = v[best_idx]
            best_prop = pd.Series({k: v[best_idx] for k, v in row.items()})
                
            return {'best_peaks': list(best_pos), 'best_prop': best_prop}
        
        # find peak based on maximum positional value
        # @staticmethod
        def find_max_index(row, Nbins=512):
            return np.nanargmax((row - np.round(Nbins/2)).abs())
        
        def gen_peak_prop_df(df_peaks, df_prop, n=3):
            ...
            # df_peaks.doppler_idx = df_peaks.doppler_idx.astype('float').astype('Int32')

            # getting the df_peaks and padding with nan to match n_peaks
            dfc = df_peaks.copy()
            df_nans = pd.DataFrame(np.nan, index=dfc.index, columns = [f'peaks_{i}' for i in range(n)])
            df_nans.loc[:, dfc.columns] = dfc.values
            df_peaks = df_nans.copy()
            del dfc, df_nans


            for c in df_prop.columns:

                df_nans = pd.DataFrame(np.nan, index=df_peaks.index, columns = [f'{c}_{i}' for i in range(n)])
                df_temp = pd.DataFrame(df_prop[c].to_list()).add_prefix(f'{c}_')
                df_nans.loc[:,df_temp.columns] = df_temp.values

                df_peaks = pd.concat([df_peaks, df_nans], axis=1)
    
                del df_temp, df_nans
            
            suffix_idx = df_peaks.columns[df_peaks.columns.str.contains(r'\d+$')].str.extract(r'(\d+)$', expand=False).astype(int)
            
            dfs = {}
            for idx in suffix_idx.unique():
                peaks = df_peaks[f'peaks_{idx}']
                peak_start = df_peaks[f'left_ips_{idx}'].apply(np.floor).copy()
                peak_end = df_peaks[f'right_ips_{idx}'].apply(np.ceil).copy()
                peak_heights = df_peaks[f'peak_heights_{idx}']
                prominences = df_peaks[f'prominences_{idx}']
                h_center = (peak_start + peak_end)/2
                v_center = (peak_heights + prominences)/2
                df_temp = pd.DataFrame(data = zip(peaks, peak_start, peak_end, peak_heights, prominences, h_center, v_center), 
                                        columns=['peaks', 'peak_start', 'peak_end', 'peak_heights', 'prominences', 'h_center', 'v_center'], index=df_peaks.index)
                dfs[f'peak_{idx}'] = df_temp                                    
        ...
        # FIXME: sometimes there are more peaks in the spectrum.need to output more peaks, therefore more vlos for post-processing
        # df_peaks, mean_peak = self.extract_best_peaks_v0(df_peaks, df_prop)
        df_pks = df_prop.apply(lambda x: find_best_peaks(x, df_peaks, n=self.inp.n_peaks), axis=1, result_type='expand')

        df_peaks = pd.DataFrame(np.nan, index=S_peak.index, columns = [f'peaks_{i}' for i in range(self.inp.n_peaks)])
        df_peaks_2 = pd.DataFrame(df_pks['best_peaks'].to_list()).add_prefix('peaks_')  
        df_peaks.loc[:, df_peaks_2.columns] = df_peaks_2.values

        df_prop = pd.DataFrame(df_pks['best_prop'].to_list())
        ...
        # find peaks based on maximum widths
            ...
            peaks_cwt = spec_flat.apply(find_peaks_cwt, widths=30, window_size=150, axis=1, result_type='reduce')        
            df_peaks[peak_mode + '_idx']  = pd.DataFrame(peaks_cwt.to_list()).iloc[:,1]
        elif peak_mode == 'nearest_mean':
            df_peaks[peak_mode + '_idx'] = df_peaks.filter(like='peaks').apply(lambda x: np.argmin(np.abs(x-mean_peak)), axis=1).astype('int16').copy()
        ...
        # dfs = {}
        # for i in range(1,4):         
        #     dfs[i] = gen_peak_prop_df(df_peaks.filter(regex=f'.*_{i}$'))
        #     dfs[i]['doppler_peak'] = df_peaks.doppler_peak
        #     dfs[i]['doppler_idx'] = df_peaks.doppler_idx
        #     if i > 1:
        #         # replace nan values in dfs if the lower than 3 peaks were found
        #         nan_counts = dfs[i].isna().sum(axis=1)
        #         dfs[i].loc[nan_counts>5,:] = dfs[1].loc[nan_counts>5,:]                
        
        return dfs

    def extract_best_peaks_v0(self, df_peaks, df_prop):
        idx_Nmax = df_peaks.count(axis=0).argmax()
        mean_peak = df_peaks.apply(np.nanmean).iloc[idx_Nmax] # FIXME: gets wrong mean
        
        # find the diff between the peaks found and the mean peak
        df_diff = df_peaks.sub(mean_peak).abs()
        # get the three smallest diff numbers
        df_diffs = df_diff.apply(lambda row: row.nsmallest(3), axis=1)
        
        df_peaks_out = pd.DataFrame()
        for c in df_prop.columns:
            df_temp = pd.DataFrame(df_prop[f'{c}'].to_list()).add_prefix(f'{c}_')
            df_peaks_out = pd.concat([df_peaks_out, df_temp], axis=1)
        df_peaks_out = pd.concat([df_peaks_out, df_peaks], axis=1)
        df_peaks = df_peaks.reindex(sorted(df_peaks.columns, key=lambda x: float(x.split('_')[-1])), axis=1)
        
        # get the peak numbers from peaks
        df_out = pd.DataFrame()
        for i in df_diffs.index:
            out = df_diffs.iloc[i,:].dropna().index.to_list() 
            idx = [int(o.split('_')[-1]) for o in out]
            
            # gett the peak properties for the index
            df_row = pd.DataFrame()
            for pk_i, n in enumerate(idx):
                df_temp = df_peaks_out.filter(regex=f'.*_{n}$').iloc[i,:].drop(index=[f'left_bases_{n}', f'right_bases_{n}', f'width_heights_{n}'])
                df_temp.index = [i[0] for i in df_temp.index.str.split('_\d$')]
                df_row = pd.concat([df_row, df_temp.add_suffix(f'_{pk_i+1}')], axis=0)
                
            df_out = pd.concat([df_out, df_row.T], axis=0)
            
        df_peaks = df_out.copy()
        return df_peaks,mean_peak

    def blade_correction(self, spec_flat, gauss, thresh=4):
        """blade correction using a func above a threshold"""
        
        idx = spec_flat[spec_flat.max(axis=1) > thresh].index
    ...
    def calc_snr(spec, peaks, method='peak'):
        ...
        var_s = P_signal
        var_n = P_noise
        abs_snr = np.abs(P_signal - P_noise) / P_noise
        snr = 10*np.log10(abs_snr, where = abs_snr>0)
        peak_power = spec[peak_id]

        return snr, var_s, var_n, peak_power

    # @profile
    def RunSpectralAnalysis(self, peak_mode='prom'):
        ...
        vlos, snr  = {}, {}
        var_s, var_n, peak_power = {}, {}, {}
        for k, df_pk in df_peaks.items():
            spec_all = self.prepare_spec(spec_real, df_pk)

            # calculate the snr based on exact peak
            snr_out = spec_all.apply(lambda s: self.calc_snr(s.filter(regex="^\d{1,3}").values, peaks=s.peak_exact, method='area'), axis=1, result_type='expand')
            # convert the snr output from above into snr, variance in signal and noise respectively
            snr[k], var_s[k], var_n[k], peak_power[k] = snr_out.iloc[:,0], snr_out.iloc[:,1], snr_out.iloc[:,2], snr_out.iloc[:,3]
            
            # convert lags into vlos
            vlos[k] = self.spectrum2vlos(spec_all.peak_exact - 256)
            
            # reindexing to the original size
            vlos[k] = vlos[k].reindex(spec_all.index)
            vlos[k].name = 'vlos'
            snr[k] = snr[k].reindex(spec_all.index)
            snr[k].name = 'snr'

        return vlos, snr, df_peaks, peak_power
    ...
    @staticmethod
    def gaussian_double_peak_distribution(x, mu1, sig1, mu2, sig2, a1, a2,  **kwargs):
        """
            Generate a theoretical double peak Gaussian distribution.

            Parameters:
            mu1 (float): Mean of the first Gaussian distribution.
            sig1 (float): Standard deviation of the first Gaussian distribution.
            mu2 (float): Mean of the second Gaussian distribution.
            sig2 (float): Standard deviation of the second Gaussian distribution.
            a1 (float): Amplitude of the first Gaussian distribution.
            a2 (float): Amplitude of the second Gaussian distribution.
            x (tuple): values along the x-axis

            Returns:
            pdf - prodbability distribution function with two gaussian peaks.        
        """
        
        plot_figure = kwargs.setdefault('plot_figure', False)

        pdf1 = a1 * np.exp(-1 * ((x-mu1)/sig1)**2)
        pdf2 = a2 * np.exp(-1 * ((x-mu2)/sig2)**2)
        pdf = pdf1  + pdf2
        
        if plot_figure:
            plt.plot(x, pdf, 'k.-')
            plt.xlabel("x-data")
            plt.ylabel("Gaussian double peak pdf")
        
        return pdf
    
    @staticmethod
    def func_lsq_fit(func, x, y, **kwargs):
        """"
        apply a gaussian double peak distribution to double peaks
        
        x - values along the x-axis
        y - noise data that is supposed to be fit to the distribution
        
        Returns:
        popt - popt will have the same number of values as arguments in the func, except the x-data
        pcov - pcov provides the covariance matrix
        """
        
        from scipy.optimize import curve_fit
        
        p0 = kwargs.setdefault('p0', [230, 15, 248, 15, 1.3, 1.3])
        
        popt, pcov = curve_fit(func, x, y, p0=p0, absolute_sigma=True)
        
        return popt, pcov     
    

    @staticmethod
    def plot_spec_heatmap(spectrum, ref_y, vlim=[400, 650], nplots = [3, 1]):
        """
            plot a spectrum with timestamp on x-axis and spectral power or derived qty along y-axis
        ...
        X,Y = np.meshgrid(spectrum.index, (spectrum.columns-256)*0.1836)
        C = ax[0].pcolormesh(X, Y, spectrum.values.T, vmin = vlim[0], vmax = vlim[1], shading = 'gouraud')
        plt.colorbar(C, ax = ax, label = 'Spectral Power [-]')
        ax[0].set(xlabel = 'Timestamp [-]', ylabel = 'v_los [-]')
        ax[0].axis([spectrum.index.min(), spectrum.index.max(), -10, 10])
        ax[1].plot(spectrum.index, ref_y)
        ax[1].set(ylabel='z [m]')
        ax[1].axis([spectrum.index.min(), spectrum.index.max(), ref_y.min(), ref_y.max()])
        ax[0].legend()
        plt.show()
    ...
    def prepare_spec(self, spec_real, df_peaks):
        ...
        spec_all = pd.concat([spec_all, peak+256], axis=1)
...
if __name__ == "__main__":
    ...
    if debug == False:
        ...
        spec_all = sa.prepare_spec(spec_real, df_peaks)

        # calculate the snr based on exact peak
        snr = spec_all.apply(lambda s: sa.calc_snr(s.filter(regex="^\d{1,3}").values, peaks=s.peaks, method='area'), axis=1, result_type='expand')
        peak = spec_all.apply(lambda s: sa.FnFindPeaks_exact(s.filter(regex="^\d{1,3}"),s.filter(like='peak')), axis=1, result_type='expand')
        # # derive the vlos from peak positions
        vlos = sa.spectrum2vlos(peak.iloc[:,0])
