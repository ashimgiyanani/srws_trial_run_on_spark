...
class ProcessSRWS:
    ...
    def MotorToCoordxyz(self, m1, m2, m3):
        ...
        xcalc = d * np.cos(phi1) * np.sin(theta) + L * (
            np.cos(phi1) * np.cos(theta) * np.sin(theta) - np.sin(theta) *
            (np.sin(phi1) * np.sin(phi2) -
             np.cos(phi1) * np.cos(phi2) * np.cos(theta)))
        ycalc = d * np.sin(phi1) * np.sin(theta) + L * (
            np.cos(theta) * np.sin(phi1) * np.sin(theta) + np.sin(theta) *
            (np.cos(phi1) * np.sin(phi2) +
             np.cos(phi2) * np.cos(theta) * np.sin(phi1)))
        zcalc = d * np.cos(theta) + L * (
    ...
    def CoordxyzToMotor(self, x, y, x0, theta, d):
        ...
        def rotation_solve(x, *data):
            y, theta, d = data
            F = np.array([
                d * np.cos(x[0]) * np.sin(theta) + x[2] *
                (np.cos(x[0]) * np.cos(theta) * np.sin(theta) - np.sin(theta) *
                 (np.sin(x[0]) * np.sin(x[1]) - np.cos(x[0]) * np.cos(x[1]) *
                  np.cos(theta))), d * np.sin(x[0]) * np.sin(theta) + x[2] *
                (np.sin(theta) *
                 (np.cos(x[0]) * np.sin(x[1]) +
                  np.cos(x[1]) * np.sin(x[0]) * np.cos(theta)) +
                 np.cos(theta) * np.sin(x[0]) * np.sin(theta)),
                d * np.cos(theta) + x[2] * (np.cos(theta)**2 - np.cos(x[1]) *
                                            (np.sin(theta)**2))
    ...
    def CampaignConstants(self, coord, plot_figure):
        ...
        if self.inp.plot_figure == True:
            ax = df_tower_flance.plot(x="Easting",
                                      y="Northing",
                                      kind="scatter")
        ...
        alpha = np.arctan2(D[1], D[0])
        Center_coord = [
            np.array(TW_center)[0][1] - L * np.cos(alpha),
            np.array(TW_center)[0][2] - L * np.sin(alpha)
        ]
        x_t = [
            Center_coord[0] + L * np.cos(alpha),
            Center_coord[0] - L * np.cos(alpha)
        ]
        y_t = [
            Center_coord[1] + L * np.sin(alpha + np.pi),
            Center_coord[1] - L * np.sin(alpha + np.pi)
        ]
        ...
        mask = [
            'TOP CENTER', 'WS1', 'WS2', 'WS3', 'Tower_center', 'BALL CENTER'
        ]
        if plot_figure == True:
            fig, ax1 = plt.subplots()
            fig.set_size_inches(18.5, 10.5)
            df.loc[['TOP CENTER', 'Tower_center']].plot(x="Easting",
                                                        y="Northing",
                                                        ax=ax1,
                                                        color='black')
            ax1.scatter(Center_coord[0], Center_coord[1], color='red')
            df.loc[mask].plot(x="Easting",
                              y="Northing",
                              kind='scatter',
                              ax=ax1)
            ax1.plot(x_t, y_t, color='black')
            ax1.legend([
                'Coordinate system x-axis', 'Coordinate system y-axis',
                'Center of coordinatesystem', 'Leica positions'
            ])
            ax1.axis('scaled')
        ...
        x_new, y_new, z_new = ProcessSRWS.Rotate_and_translate(
            x, y, z, [Center_coord[0], Center_coord[1], 0], 0,
            -alpha + np.pi / 2)
        ...
        if self.inp.plot_figure == True:
            ...
            df_rotate.loc[mask].plot(x="Easting",
                                     y="Northing",
                                     kind='scatter',
                                     ax=ax1)
    ...
    def Read_SRWS_bin(
            ...
            **kwargs) -> Tuple[np.ndarray, pd.DataFrame, pd.DataFrame]:
        """
        ...
        dt = np.dtype([
            ...
            (
                'ActPulse 1', np.int32
            ),  # used as a channel to send a trigger pulse when working in a windtunnel
            ('ActPulse 2', np.int32),
            ('ActPulse 3', np.int32),
            ('ModeVar', np.int32),
            ('Status 1', np.uint8),
            ...
            (
                'AverageCount 1', np.uint16
            ),  # no. of spectra averaged in each sample, high when sampling frequency is low
            ('vlos 1', np.float32),
            ('MaximumValue 1', np.float32),
            ('TotalPower 1', np.int32),
            ('Qualitydata 1', np.float32),
            ('Beta 1', np.float32
             ),  # calculated from the spectrum, to do something with the noise
        ...
        if mode == 'basic':
            ...
            print(
                'Input the mode of data extraction: options (basic, standard, all)'
            )
        ...
        if write_spec == True:
            ...
            for name in spec_data.dtype.names:
                ...
                if ((df_temp**2).sum(axis=1) == 0).sum() == len(df_temp) or ((
                    (df_temp**2).sum() == 0).sum() > df_temp.shape[1] / 2):
                    max_name = name.replace('Spectrum', 'MaximumValue')
                    if (df[max_name].sum() == 0) or (df[max_name].mean() > 2**
                                                     16):
            ...
            max_names = [
                s.replace('Spectrum', 'MaximumValue') for s in spec_names
            ]
            max_new_names = dict(zip(
                df.filter(like='Maxi').columns, max_names))
        ...
        try:
            if df.index.stop >= 19000:
                T_gen = pd.date_range(T0,
                                      periods=df.index.stop,
                                      freq=str(int(1e9 * 60 / df.index.stop)) +
                                      'ns')
            else:
                T_gen = pd.date_range(T0,
                                      periods=df.index.stop,
        ...
        spec_df = spec_df.set_index(df.index)

        # convert to UTC timestamps
        df.index = df.index.tz_convert('UTC')
        spec_df.index = df.index.tz_convert('UTC')
    ...
    def ReconstructUV(self, V_1, V_2, theta_1, theta_2, phi_1, phi_2):
        ...
        denom = np.cos(phi_1) * np.cos(phi_2) * (
    ...
    def ReconstructUVW(self,
        ...
        u = (V_1 * (beta_2 * np.sin(phi_3) - beta_3 * np.sin(phi_2)) + V_2 *
             (beta_3 * np.sin(phi_1) - beta_1 * np.sin(phi_3)) + V_3 *
             (beta_1 * np.sin(phi_2) - beta_2 * np.sin(phi_1))) / denom

        v = (V_1 * (alpha_3 * np.sin(phi_2) - alpha_2 * np.sin(phi_3)) + V_2 *
             (alpha_1 * np.sin(phi_3) - alpha_3 * np.sin(phi_1)) + V_3 *
             (alpha_2 * np.sin(phi_1) - alpha_1 * np.sin(phi_2))) / denom

        w = (a * V_1 + b * V_2 + c * V_3) / denom
        return u, v, w

    def ReconstructUVW_pedersen(self, V_1, V_2, V_3, theta_1, theta_2, theta_3,
                                phi_1, phi_2, phi_3):
        ...
        u = (V_1 * (alpha_2 * np.sin(phi_3) - alpha_3 * np.sin(phi_2)) + V_2 *
             (alpha_3 * np.sin(phi_1) - alpha_1 * np.sin(phi_3)) + V_3 *
             (alpha_1 * np.sin(phi_2) - alpha_2 * np.sin(phi_1))) / denom

        v = (V_1 * (beta_3 * np.sin(phi_2) - beta_2 * np.sin(phi_3)) + V_2 *
             (beta_1 * np.sin(phi_3) - beta_3 * np.sin(phi_1)) + V_3 *
             (beta_2 * np.sin(phi_1) - beta_1 * np.sin(phi_2))) / denom
    ...
    def srws2df(self, inp, file, campaign_constants, Nfiles):
        ...
        if inp.srws.use_corrected_vlos == True:
            ...
            _, raw_df, spec_df = self.Read_SRWS_bin(filename,
                                                    mode='all',
                                                    write_spec=True)
            ...
            objs = [
                SpectralAnalysis(spec_df['Spectrum {}'.format(i)], inp.spec,
                                 raw_df['MaximumValue {}'.format(i)])
                for i in range(1, 4)
            ]
            with Parallel(n_jobs=3) as parallel:
                delayed_funcs = [
                    delayed(lambda x: x.RunSpectralAnalysis())(obj)
                    for obj in objs
                ]
                run_df = parallel(delayed_funcs)

            vlos_df = pd.concat([pd.DataFrame(df[0]) for df in run_df],
                                ignore_index=True,
                                axis=1).set_axis(vlos_names,
                                                 axis=1).reset_index(drop=True)
            snr_df = pd.concat([pd.DataFrame(df[1]) for df in run_df],
                               ignore_index=True,
                               axis=1).set_axis(snr_names,
                                                axis=1).reset_index(drop=True)

            Data_raw = pd.concat(
                [raw_df.reset_index(drop=True), vlos_df, snr_df],
                axis=1).set_index(raw_df.index)
        else:
            # Step 2b: read the raw data file
            vlos_names = ['vlos {}'.format(i) for i in range(1, 4)]
            _, Data_raw, spec_df = self.Read_SRWS_bin(filename,
                                                      mode='all',
                                                      write_spec=True)
        ...
        Data_raw = qc.detect_time_gaps(dt_in=inp.srws.dt,
                                       channel_name=None,
                                       correct_gaps=False)

        # detect outliers in ws range
        Data_raw = qc.detect_outliers_ws_range(channel_names=ws_names,
                                               ranges=[-35, 35])
        ...
        x1, y1, z1 = self.MotorToCoordxyz(Data_raw['ActPosition_pm1 1'],
                                          Data_raw['ActPosition_pm2 1'],
                                          Data_raw['ActPosition_fm3 1'])
        x2, y2, z2 = self.MotorToCoordxyz(Data_raw['ActPosition_pm1 2'],
                                          Data_raw['ActPosition_pm2 2'],
                                          Data_raw['ActPosition_fm3 2'])
        x3, y3, z3 = self.MotorToCoordxyz(Data_raw['ActPosition_pm1 3'],
                                          Data_raw['ActPosition_pm2 3'],
                                          Data_raw['ActPosition_fm3 3'])

        # Step 2c: commanded positions for BowTie pattern
        xo1, yo1, zo1 = self.MotorToCoordxyz(Data_raw['ComPosition_pm1 1'],
                                             Data_raw['ComPosition_pm2 1'],
                                             Data_raw['ComPosition_fm3 1'])
        xo2, yo2, zo2 = self.MotorToCoordxyz(Data_raw['ComPosition_pm1 2'],
                                             Data_raw['ComPosition_pm2 2'],
                                             Data_raw['ComPosition_fm3 2'])
        xo3, yo3, zo3 = self.MotorToCoordxyz(Data_raw['ComPosition_pm1 3'],
                                             Data_raw['ComPosition_pm2 3'],
                                             Data_raw['ComPosition_fm3 3'])

        # Step 3a: Coordinate in Mikael-coordinates system
        # azi = (azi + np.pi) % (2*np.pi)
        xrot1, yrot1, zrot1 = ProcessSRWS.scanner2globelCoord(
            x1, y1, z1, WS1, elev[0], azi[0])
        xrot2, yrot2, zrot2 = ProcessSRWS.scanner2globelCoord(
            x2, y2, z2, WS2, elev[1], azi[1])
        xrot3, yrot3, zrot3 = ProcessSRWS.scanner2globelCoord(
            x3, y3, z3, WS3, elev[2], azi[2])
        ...
        theta1_rot, phi1_rot, _ = ProcessSRWS.calc_angles(
            WS1[0] - xrot1, WS1[1] - yrot1, WS1[2] - zrot1)
        theta2_rot, phi2_rot, _ = ProcessSRWS.calc_angles(
            WS2[0] - xrot2, WS2[1] - yrot2, WS2[2] - zrot2)
        theta3_rot, phi3_rot, _ = ProcessSRWS.calc_angles(
            WS3[0] - xrot3, WS3[1] - yrot3, WS3[2] - zrot3)
        ...
        )  # 26.01.2024 Paul Meyer found that the reconstructed wind components are 90° offseted i.e. vx is towards AD8 wt and vy is towards west
        # getting the radial wind speeds
        V1, V2, V3 = Data_raw.loc[:, ws_names[0]], Data_raw.loc[:, ws_names[
            1]], Data_raw.loc[:, ws_names[2]]
        vx, vy, vz = self.ReconstructUVW(V1, V2, V3, theta_rot[0],
                                         theta_rot[1], theta_rot[2],
                                         phi_rot[0], phi_rot[1], phi_rot[2])
        [vx, vy, vz] = np.dot(Rz, [vx, vy, vz])
        # Step 4b:  wind vector from beam 1 and beam 2
        vx12, vy12 = self.ReconstructUV(V1, V2, theta_rot[0], theta_rot[1],
                                        phi_rot[0], phi_rot[1])
        [vx12, vy12, vz] = np.dot(Rz, [vx12, vy12, vz])
        # wind vector from beam 2 and beam 3
        vx23, vy23 = self.ReconstructUV(V2, V3, theta_rot[1], theta_rot[2],
                                        phi_rot[1], phi_rot[2])
        [vx23, vy23, vz] = np.dot(Rz, [vx23, vy23, vz])
        # wind vector from beam 3 and beam 1
        vx31, vy31 = self.ReconstructUV(V3, V1, theta_rot[2], theta_rot[0],
                                        phi_rot[2], phi_rot[0])
        ...
        # getting the radial wind speeds
        V1_pp, V2_pp, V3_pp = Data_raw.loc[:, vlos_names[
            0]], Data_raw.loc[:, vlos_names[1]], Data_raw.loc[:, vlos_names[2]]
        vx_pp, vy_pp, vz_pp = self.ReconstructUVW(V1_pp, V2_pp, V3_pp,
                                                  theta_rot[0], theta_rot[1],
                                                  theta_rot[2], phi_rot[0],
                                                  phi_rot[1], phi_rot[2])
        [vx_pp, vy_pp, vz_pp] = np.dot(Rz, [vx_pp, vy_pp, vz_pp])
        # Step 4b:  wind vector from beam 1 and beam 2
        vx12_pp, vy12_pp = self.ReconstructUV(V1_pp, V2_pp, theta_rot[0],
                                              theta_rot[1], phi_rot[0],
                                              phi_rot[1])
        [vx12_pp, vy12_pp, vz_pp] = np.dot(Rz, [vx12_pp, vy12_pp, vz_pp])
        # wind vector from beam 2 and beam 3
        vx23_pp, vy23_pp = self.ReconstructUV(V2_pp, V3_pp, theta_rot[1],
                                              theta_rot[2], phi_rot[1],
                                              phi_rot[2])
        [vx23_pp, vy23_pp, vz_pp] = np.dot(Rz, [vx23_pp, vy23_pp, vz_pp])
        # wind vector from beam 3 and beam 1
        vx31_pp, vy31_pp = self.ReconstructUV(V3_pp, V1_pp, theta_rot[2],
                                              theta_rot[0], phi_rot[2],
                                              phi_rot[0])
        ...
        from scipy.spatial.transform import Rotation as R
        alpha = -9  # source geo Daten Total station survey, changed from +9 to -9 deg [29.06.2023]
        ...
        [u, v, w] = np.dot(
            Rz,
            [vx, vy, vz])  # TODO: check if this results in the error of u/v
        ...
        vz12_pp, vz23_pp, vz31_pp = vx12.copy() * 0, vx23.copy(
        ) * 0, vx31.copy() * 0
        [u_pp, v_pp, w_pp
         ] = np.dot(Rz,
                    [vx_pp, vy_pp, vz_pp
                     ])  # TODO: check if this line results in the error of u/v
        ...
        gamma[~nan_mask] = (
            np.rad2deg(np.arctan2(vx[~nan_mask], vy[~nan_mask])) + 90) % 360
        ...
        gamma_pp[~nan_mask] = (np.rad2deg(
            np.arctan2(vx_pp[~nan_mask], vy_pp[~nan_mask])) + 90) % 360
        ...
        _, _, _, _, _, k = FnFindNearestGridPts(X,
                                                Y,
                                                Z,
                                                xlg,
                                                ylg,
        ...
        fe_R = (dR1 + dR2 + dR3) / 3
        fe_T = pd.DataFrame(np.transpose([dT1, dT2, dT3]),
                            columns=['dT1', 'dT2', 'dT3']).apply(circmean,
                                                                 axis=1)
        fe_P = pd.DataFrame(np.transpose([dP1, dP2, dP3]),
                            columns=['dT1', 'dT2', 'dT3']).apply(circmean,
                                                                 axis=1)
        ...
        wfr_all = pd.DataFrame(listTuples,
                               columns=nameTuples).reset_index(drop=True)
        Data = pd.concat([Data_raw, wfr_all], axis=1).set_index('index',
                                                                drop=True)
        ...
        if inp.generate_stats:
            circ_cols = [
                'gamma', 'gamma_pp', 'psi', 'psi_pp', 'thetar1', 'thetar2',
                'thetar3', 'phir1', 'phir2', 'phir3', 'theta1', 'theta2',
                'theta3', 'phi1', 'phi2', 'phi3', 'fe_T', 'fe_P'
            ]
            nondefault_cols = ['gamma', 'gamma_pp']
            Data.update(Data.filter(regex="gamma|psi").apply(np.deg2rad))
            Data_mean = pd.DataFrame(
                pa.multiple_stats_on_df(Data,
                                        circular_data_cols=circ_cols,
                                        circ_pi2pi=nondefault_cols)).T
            ...
            Data_mean = Data_mean.reset_index(drop=True).set_index('Tavg',
                                                                   drop=True)
        ...
        listTuples = list(
            zip(vx, vy, vz, Vvec, Vhorz, gamma, psi, vx12, vy12, Vhorz12, vx23,
                vy23, Vhorz23, vx31, vy31, Vhorz31, vx_pp, vy_pp, vz_pp,
                vx12_pp, vy12_pp, vx23_pp, vy23_pp, vx31_pp, vy31_pp, Vvec_pp,
                Vhorz_pp, gamma_pp, psi_pp, Vhorz12_pp, Vhorz23_pp, Vhorz31_pp,
                fe_R, fe_T, fe_P, dZ_eff, dZ[0], dZ[1], dZ[2], dL, Leff, u, v,
                w, u12, v12, w12, u23, v23, w23, u31, v31, w31, u_pp, v_pp,
                w_pp, u12_pp, v12_pp, w12_pp, u23_pp, v23_pp, w23_pp, u31_pp,
                v31_pp, w31_pp, Data_raw['dt 1'], Data_raw['dt 2'],
                Data_raw['dt 3'], Data_raw.qc1, Data_raw.qc2))
        nameTuples = [
            'vx', 'vy', 'vz', 'Vvec', 'Vhorz', 'gamma', 'psi', 'vx12', 'vy12',
            'Vhorz12', 'vx23', 'vy23', 'Vhorz23', 'vx31', 'vy31', 'Vhorz31',
            'vx_pp', 'vy_pp', 'vz_pp', 'vx12_pp', 'vy12_pp', 'vx23_pp',
            'vy23_pp', 'vx31_pp', 'vy31_pp', 'Vvec_pp', 'Vhorz_pp', 'gamma_pp',
            'psi_pp', 'Vhorz12_pp', 'Vhorz23_pp', 'Vhorz31_pp', 'fe_R', 'fe_T',
            'fe_P', 'dZ_eff', 'dZ1', 'dZ2', 'dZ3', 'dL', 'Leff', 'u', 'v', 'w',
            'u12', 'v12', 'w12', 'u23', 'v23', 'w23', 'u31', 'v31', 'w31',
            'u_pp', 'v_pp', 'w_pp', 'u12_pp', 'v12_pp', 'w12_pp', 'u23_pp',
            'v23_pp', 'w23_pp', 'u31_pp', 'v31_pp', 'w31_pp', 'dt1', 'dt2',
            'dt3', 'qc1', 'qc2'
        ]
        wfr = pd.DataFrame(listTuples,
                           columns=nameTuples).reset_index(drop=True)
        ...
        if not idx_hub[0].any():
            mean_pt = [
                pa.find_nearest(X, xlg.mean()),
                pa.find_nearest(Y, ylg.mean()),
                pa.find_nearest(Z, zlg.mean())
            ]
            idx_hub = np.where(np.sum(k == mean_pt, axis=1) == 3)
        df_H = wfr.iloc[np.ravel(idx_hub), :].set_index(
            Data.index[np.ravel(idx_hub)])
        ...
        if inp.write_csv == True:
            ...
            self.write_srws2csv(df_bowtie,
                                csv_name='srws_avg_bowtie_ID6362.csv')
    ...
    def add_metadata(files):
        ...
        from sqlalchemy.orm import (relationship, declarative_base, registry,
                                    ...
                                    backref, mapped_column)
        from sqlalchemy.ext.serializer import loads, dumps
        # from enumerations import *
        from metadata_helpers import (get_CampaignInfo, get_guiData, as_dict,
                                      json_serialize, qc_flags_basic)
        from metadata_tables import (DatasetInfo, DataFile, UserInterface,
                                     Network, Site, WindFarm, InstrumentSpecs,
                                     LidarConfig, AggQualityControl,
                                     DataAccessLayer)
        from datetime import datetime
        dateformat = "%Y-%m-%dT%H:%M:%SZ"
        ...
        meta_campaign = get_CampaignInfo(
        )  # duration and resolution not correct
        meta_dsi = DatasetInfo(
            ...
            subject="wind_3d",
            description=
            "Measurement of 3D wind fields in front of Adwen AD8-180 wind turbine at Testfeld BHV, measurement performed by Fraunhofer IWES in cooperation with DTU between October 2021 and June 2022",
            references=
        ...
        meta_net = Network(daylight_saving=True, data_trans_freq="daily")
        ...
        meta_dataf = DataFile(
            ...
            program="Bowtie1",
            title=
            "radial velocities and spectrum data from three synchronized SRWS",
        )
        meta_site = Site(
            geospatial_bounds=
            "polygon((8.575881, 53.501206, (8.579721, 53.501168, (8.580494, 53.506630, (8.573821, 53.505265))",
        ...
        meta_srws = InstrumentSpecs(
            ...
            safety_check=
            "safe for operation with special considerations (see report)",
        ...
        meta_objs = [
            meta_dsi, meta_ui, meta_net, meta_dataf, meta_site, meta_wf,
            meta_srws, meta_lidcfg, meta_aqc
        ]
        ...
        # dal.session.flush()
        dal.session.commit()
        query = select(DatasetInfo, UserInterface, Network, DataFile, Site,
                       WindFarm, InstrumentSpecs, LidarConfig,
                       AggQualityControl)
        ...
        for r in ResultSet[0]:
            ...
            json_obj = json.dumps(dct,
                                  indent=4,
                                  default=lambda x: json_serialize(x))
    ...
    def write_srws2csv(self, df, csv_name):
        ...
        """
        path = os.path.join(self.inp.target_path, 'data', 'Bowtie1', 'csv',
                            csv_name)
    ...
    def FnConvertRawData(self, inp):
        ...
        # Step 1: Get measurement campaign information
        campaign_constants = self.CampaignConstants(inp.srws.coord,
                                                    inp.plot_figure)

        # read the number of files to be read, unzip if necessary
        Nfiles = 0
        files = glob.glob(os.path.join(inp.srws.path.root, inp.srws.regStr),
                          recursive=True)

        # Initialization
        DATA, Data_Mean, Data_Std, Data_Max = pd.DataFrame(), pd.DataFrame(
        ), pd.DataFrame(), pd.DataFrame()
        Data_Min, DF_H, DF_bowtie, DF_m, spec_DF = pd.DataFrame(
        ), pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
        data_raw = pd.DataFrame()
        data_hub = pd.DataFrame()
        data_1min = pa.struct()
        base_path = os.path.join(self.inp.target_path, 'data', 'Bowtie1',
                                 'parquet')
        ...
        for file in sorted(files[:]):
            ...
            try:
                Nfiles += 1
                Data, df_H, df_bowtie, df_m, Data_mean, Data_std, Data_min, Data_max, spec_df, meta_ds = self.srws2df(
                    inp, file, campaign_constants, Nfiles)
                print('[{0}]: Files {1}/{2} {3} completed'.format(
                    pa.now(), Nfiles, len(files), filename[-20:]))
            except OverflowError:
                Nfiles += 1
                print('[{0}]: File {1}/{2} {3} skipped (OverflowError)'.format(
                    pa.now(), Nfiles, len(files), filename[-20:]))
            ...
            if inp.srws.merge == True:  # dataframe concatenate yes=True, no=False
                ...
                filepath = os.path.join(
                    f'{base_path}',
                    f'data_{files_tstart.strftime("%Y%m%dT%H%M%S%Z%z")}.parq')
                # write to parquet files
                pa.write_parquet(filepath, Data)
                pa.write_fastparquet(
                    os.path.join(f'{base_path}', f'df_H.parq'), df_H)
                pa.write_fastparquet(
                    os.path.join(f'{base_path}', f'df_bowtie.parq'), df_bowtie)
                pa.write_fastparquet(
                    os.path.join(f'{base_path}', f'df_m.parq'), df_m)
            ...
            pa.write_fastparquet(
                os.path.join(
                    f'{base_path}',
                    f'df_H_{files_tstart.strftime("%Y%m%dT%H%M%S%Z%z")}.parq'),
                DF_H)
            pa.write_fastparquet(
                os.path.join(
                    f'{base_path}',
                    f'df_bowtie_{files_tstart.strftime("%Y%m%dT%H%M%S%Z%z")}.parq'
                ), DF_bowtie)
            pa.write_fastparquet(
                os.path.join(
                    f'{base_path}',
                    f'df_m_{files_tstart.strftime("%Y%m%dT%H%M%S%Z%z")}.parq'),
                DF_m)
            ...
            regStr = "^(?!qc).*\s?\D{}r?$"
            indexStr = 'srws_nr{}'
            nc_base_path = os.path.join(self.inp.target_path, 'data',
                                        'Bowtie1', 'netcdf')
        ...
        if (inp.write_parquet == True) & (inp.srws.merge == True):
            pa.write_parquet(
                os.path.join(self.inp.target_path, 'data', 'Bowtie1',
                             'parquet', f'data_raw.parq'), data_raw)
            # pa.write_parquet(os.path.join(self.inp.target_path,'data','parquet',f'data_1min_mean.parq'), data_1min.mean)
            pa.write_parquet(
                os.path.join(self.inp.target_path, 'data', 'Bowtie1',
                             'parquet', f'data_hub.parq'), data_hub)
            print('[{0}]: Saved srws to parquet files'.format(pa.now()))

        # adding metadata to the srws files
        if self.inp.add_metadata:
            ds_raw = ProcessSRWS.add_metadata_srws(
                ds_raw,
                nc_path=os.path.join(
                    f"{nc_base_path}",
                    f'ds_raw_{files_tstart.strftime("%Y%m%dT%H%M%S%Z%z")}.nc'))
            ds_H = ProcessSRWS.add_metadata_srws(
                ds_H,
                nc_path=os.path.join(
                    f"{nc_base_path}",
                    f'ds_H_{files_tstart.strftime("%Y%m%dT%H%M%S%Z%z")}.nc'))
            ds_bowtie = ProcessSRWS.add_metadata_srws(
                ds_bowtie,
                nc_path=os.path.join(
                    f"{nc_base_path}",
                    f'ds_bowtie_{files_tstart.strftime("%Y%m%dT%H%M%S%Z%z")}.nc'
                ))
            ds_m = ProcessSRWS.add_metadata_srws(
                ds_m,
                nc_path=os.path.join(
                    f"{nc_base_path}",
                    f'ds_m_{files_tstart.strftime("%Y%m%dT%H%M%S%Z%z")}.nc'))
    ...
    def mp_FnConvertRawData(self, inp):
        ...
        # Initialization
        DATA, DF_H, DF_bowtie, DF_m = pd.DataFrame(), pd.DataFrame(
        ), pd.DataFrame(), pd.DataFrame()
        data_raw = pd.DataFrame()
        data_hub = pd.DataFrame()
        data_1min = pa.struct()
        ...
        Nchunks, Nfiles = 0, 0
        base_path = os.path.join(inp.target_path, 'data', 'srws_data',
                                 'Bowtie1', 'parquet')
        all_files = glob.glob(os.path.join(inp.srws.path.root,
                                           inp.srws.regStr),
                              recursive=True)
        # remove completed files from the files to be converted
        completed_files_duobook = glob.glob(
            os.path.join(base_path, 'data*.parq'))
        completed_files_onedrive = glob.glob(
            os.path.join(inp.workDir, 'data', 'Bowtie1', 'parquet',
                         'data*.parq'))
        completed_files = completed_files_duobook + completed_files_onedrive
        completed_dates = [
            os.path.splitext(os.path.basename(cf).split('_')[1])[0]
            for cf in completed_files
        ]
        with open(inp.srws.path.finished_files, 'r') as fp:
            finished_dates = fp.read().splitlines()
        finished_dates = [
            pd.to_datetime(fd, utc=True).strftime("%Y%m%dT%H%M%S%Z%z")
            for fd in finished_dates
        ]
        ...
        if error_dates:
            error_dates = [
                pd.to_datetime(e, utc=True).strftime("%Y%m%dT%H%M%S%Z%z")
        ...
        chunks = [
            all_files[i:i + roll_chunks]
            for i in range(0, len(all_files), roll_chunks)
        ]
        if len(all_files) <= 20:
            Njobs = 1
        else:
        ...
        for files in chunks:
            ...
            try:
                with Parallel(n_jobs=Njobs, timeout=1800) as parallel:
                    delayed_funcs = [
                        delayed(self.srws2df)(inp, file, campaign_constants,
                                              Nfiles)
                        for file in sorted(files[:])
                    ]
                    run_df = parallel(delayed_funcs)
                    ...
                    print(
                        f'[{pa.now()}]: Chunk {Nchunks}/{len(chunks)} {files[-1][-20:]} completed'
                    )

                # TODO convert this into a generator output, will need a generator function
                for i, df_i in enumerate(run_df):
                    files_tstart = parser.parse(
                        os.path.basename(files[i])[-20:]).astimezone(
                            timezone.utc)
                    filepath = os.path.join(
                        f'{base_path}',
                        f'data_{files_tstart.strftime("%Y%m%dT%H%M%S%Z%z")}.parq'
                    ...
                    # adding metadata to the srws files TODO: perfrom this step as in FLOW dataset
                    if self.inp.add_metadata:
                        nc_base_path = os.path.join(self.inp.target_path,
                                                    'data', 'srws_data',
                                                    'Bowtie1', 'netcdf')
                        ds_raw = ProcessSRWS.add_metadata_srws(
                            ds_raw,
                ...
                FileOperations.write_paths_to_file(
                    inp.srws.path.finished_files,
                    [os.path.basename(f) for f in files])
                print(
                    f"[{pa.now()}]: Adding {os.path.basename(files[0])}-{os.path.basename(files[-1])} to finished_files.txt"
                )

                # del Data, df_H, df_bowtie, df_m
            except Exception as e:
                FileOperations.write_paths_to_file(
                    inp.srws.path.error_files,
                    [os.path.basename(f) for f in files])
                print(
                    f"[{pa.now()}]: Adding {os.path.basename(files[0])}-{os.path.basename(files[-1])} to error_files.txt"
                )
    ...
    def create_nc_groups(ds, groups, group_vars, nc_path):
        ...
        ds.drop_vars(ds.data_vars).to_netcdf(nc_path,
                                             "w",
                                             format="NETCDF4",
                                             engine="netcdf4")
        for g in groups:
    ...
    def add_metadata_srws(ds, nc_path, grouping=True):
        ...
        metadata_xl_path = r"../data/metadata_spark.xlsx"
        sheet = "Variables"
        ...
        srws_variables['valid_range'] = srws_variables.valid_range.apply(
            qc.string_eval)
        # using set theory to remove some useless variables
        valid_variables = (ds.variables.mapping.keys() -
                           set({ky
                                for ky in ds.sizes.keys()}))
        for v in sorted(valid_variables):
            idx = srws_variables[srws_variables.name == v].index
            flg = qc.qc_range_testing(
                ds[v].median().values,
                srws_variables.loc[idx, 'valid_range'].values[0])
        ...
        srws_variables['data_type'] = srws_variables.data_type.apply(
            lambda x: 'np.{}'.format(x))
        for s in srws_variables.name:
            try:
                ds[s].attrs = srws_variables[srws_variables.name == s].iloc[
                    0, :].to_dict()
            except KeyError:
                continue
        ...
        if grouping == True:
            ...
            dtu_raw_vars = set(
                [v for v in dtu_raw_vars if v in ds.variables.mapping.keys()])
            iwes_pp_vars = set({
                ky: v
                for ky, v in ds.variables.items() if ky.endswith('_pp')
            })
            dtu_pp_vars = ds.variables.mapping.keys(
            ) - iwes_pp_vars - dtu_raw_vars - set(
                {ky
                 for ky in ds.sizes.keys()})
...
if __name__ == "__main__":
    ...
    inp.srws.path.root = r"../data/srws_data"
    # inp.srws_path_root= r"z:\Projekte\112933-HighRe\20_Durchfuehrung\OE410\SRWS\Data\Bowtie1\2021\11\02"
    inp.srws.coord = r'../data/BREMERHAVEN TOT.txt'
    inp.srws.regStr = "**/*2022-04-26T185*[!.zip][!.txt]"  #
    ...
    inp.tstart = datetime.strptime(
        '2021-11-01_12-00-00', '%Y-%m-%d_%H-%M-%S'
    )  # Select start date in the form yyyy-mm-dd_HH-MM-SS
    inp.tend = datetime.strptime(
        '2021-11-01_14-59-00', '%Y-%m-%d_%H-%M-%S'
    )  # Select start date in the form yyyy-mm-dd_HH-MM-SS
    inp.add_metadata = True
    inp.target_path = r"../"
    inp.srws.path.error_files = r"../data/srws_data/error_files.txt"
    inp.srws.path.finished_files = r"../data/srws_data/finished_files.txt"
    ...
    if inp.pickle == True:
        ppsData = pd.read_pickle(
            os.path.join(inp.workDir, 'Bowtie1', 'ppsData.pickle'))
        cups_1min = pd.read_pickle(
            os.path.join(inp.workDir, 'Bowtie1', 'cups_1min.pickle'))
        sonics_1min = pd.read_pickle(
            os.path.join(inp.workDir, 'Bowtie1', 'sonics_1min.pickle'))
        data_1min = pd.DataFrame(
            pd.read_pickle(
                os.path.join(inp.workDir, 'Bowtie1', 'data_1min_mean.pickle')))
    else:
    ...
    inp.tstart = datetime.strptime(
        '2022-04-26_16-00-00', '%Y-%m-%d_%H-%M-%S'
    )  # Select start date in the form yyyy-mm-dd_HH-MM-SS
    inp.tend = datetime.strptime(
        '2022-04-26_17-00-00', '%Y-%m-%d_%H-%M-%S'
    )  # Select start date in the form yyyy-mm-dd_HH-MM-SS
    pps = PostProcess_sonics(inp)
    ppsData = pps.read_sonics(inp.pps.regStr, inp.pps.searchStr,
                              inp.pps.dateformat, inp.pps.cols)
    ...
    _, gpo, _ = FnImportOneDas(inp.tstart, inp.tend, inp.gpo.paths,
                               inp.gpo.names, inp.gpo.sampleRate,
                               inp.gpo.folder)
    ...
    # load the class
    pps = PostProcess_sonics(inp)
    WS1, WS2, WS3, elev, azi, df_rotate = srws.CampaignConstants(
        inp.srws_coord)
    ...
    fig = go.Figure()
    id = range(0, 740)
    bowtie1 = go.Scatter3d(x=data_raw.xr1.iloc[id],
                           y=data_raw.yr1.iloc[id],
                           z=data_raw.zr1.iloc[id],
                           mode='lines',
                           name='bowtie1')
    bowtie2 = go.Scatter3d(x=data_raw.xr2.iloc[id],
                           y=data_raw.yr2.iloc[id],
                           z=data_raw.zr2.iloc[id],
                           mode='lines',
                           name='bowtie2')
    bowtie3 = go.Scatter3d(x=data_raw.xr3.iloc[id],
                           y=data_raw.yr3.iloc[id],
                           z=data_raw.zr3.iloc[id],
                           mode='lines',
                           name='bowtie3')
    WS1_pl = go.Scatter3d(x=[WS1[0], WS2[0], WS3[0]],
                          y=[WS1[1], WS2[1], WS3[1]],
                          z=[WS1[2], WS2[2], WS3[2]],
                          mode='markers',
                          name='Windscanners')
    ...
    axs[0].plot(sonics_1min.index,
                sonics_1min.Vrot1,
                'ko',
                alpha=0.4,
                label='self v1')
    axs[0].plot(ppsData.index, ppsData['v1 [m/s]'], 'k.', label='pedro v1')
    axs[0].plot(cups_1min.index,
                cups_1min.Vrot1,
                'kd',
                alpha=0.5,
                label='cups v1')
    axs[0].plot(data_1min.mean.index + timedelta(minutes=1),
                data_1min.mean.V1,
                'r+',
                alpha=0.8,
                label='srws v1')
    ...
    axs[1].plot(sonics_1min.index,
                sonics_1min.Vrot2,
                'bo',
                alpha=0.4,
                label='self v2')
    axs[1].plot(ppsData.index, ppsData['v2 [m/s]'], 'b.', label='pedro v2')
    axs[1].plot(cups_1min.index,
                cups_1min.Vrot2,
                'kd',
                alpha=0.5,
                label='cups v2')
    axs[1].plot(data_1min.mean.index + timedelta(minutes=1),
                data_1min.mean.V2,
                'r+',
                alpha=0.8,
                label='srws v2')
    ...
    axs[2].plot(sonics_1min.index,
                sonics_1min.Vrot3,
                'go',
                alpha=0.4,
                label='self v3')
    axs[2].plot(ppsData.index, ppsData['v3 [m/s]'], 'g.', label='pedro v3')
    axs[2].plot(cups_1min.index,
                cups_1min.Vrot3,
                'kd',
                alpha=0.5,
                label='cups v3')
    axs[2].plot(data_1min.mean.index + timedelta(minutes=1),
                data_1min.mean.V3,
                'r+',
                alpha=0.8,
                label='srws v3')
    ...
    axs[2].set_xlabel('Time [hh:mm] ({0}-{1})'.format(
        inp.tstart.strftime('%d.%m.%y'), inp.tend.strftime('%d.%m.%y')),
                      fontsize=18)
    # plt.xlabel('Datetime')
    plt.ylim([-7, 7])
    plt.ylabel('Vlos')
    ...
    ax = plt.gca()
    plt.plot(sonics_1min.index,
             sonics_1min.Vhorz,
             'bo-',
             alpha=0.4,
             label='sonics')
    plt.plot(data_1min.mean.index + timedelta(minutes=2),
             data_1min.hub_mean.Vhorz,
             'r.-',
             label='srws')
    ...
    plt.plot(gpo1_10min.index,
             gpo1_10min['HWS hub'],
             'bx-',
             label='nacelle_lidar_V118m')
    # plt.plot(gpo2_10min.index, gpo2_10min['HWS hub'], 'gx-', label='nacelle_lidar_V393m')
    date_fmt = DateFormatter("%H:%M")
    ax.xaxis.set_major_formatter(date_fmt)
    plt.xlabel('Time [hh:mm] ({0}-{1})'.format(inp.tstart.strftime('%d.%m.%y'),
                                               inp.tend.strftime('%d.%m.%y')),
               fontsize=18)
    ...
    ax = plt.gca()
    sonics_1min['wdir'] = (
        np.rad2deg(np.arctan2(sonics_1min.U, sonics_1min.V)) + 180) % 360
    plt.plot(sonics_1min.index,
             sonics_1min.wdir,
             'bo-',
             alpha=0.4,
             label='sonics')
    plt.plot(data_1min.mean.index + timedelta(minutes=1),
             (data_1min.hub_mean.gamma) + 9,
             'r.-',
             label='srws')
