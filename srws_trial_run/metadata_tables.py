# Remarks:
# modify the datatables to match the ISO keywords types:https://wiki.esipfed.org/Documenting_Keywords [Future v2.0]
# add def __init__(self, parameters...) in each class refer: https://stackoverflow.com/questions/2047814/is-it-possible-to-store-python-class-objects-in-sqlite
...
class User(Base):
    ...
    url = mapped_column(
        String(),
        default="https://fraunhofer.sharepoint.com/sites/IWES-OE440/SitePages/en/OE-443.aspx",
...
class DatasetInfo(Base):
    ...
    acknowledgement = mapped_column(
        String(), 
        default = None,
        comment = "A place to acknowledge various types of support for the project that produced this data",
        nullable=True,
        )
    cdm_data_type = mapped_column(
        Enum(cdmEnum), 
        default = 'ts',
        comment = "The data type, as derived from Unidata's Common Data Model Scientific Data types and understood by THREDDS. (This is a THREDDS dataType, and is different from the CF NetCDF attribute 'featureType', which indicates a Discrete Sampling Geometry file in CF.)",
        nullable=False,
        )
    ...
    coverage_content_type = mapped_column(
        Enum(contentEnum), 
        default = "physical",
        comment = "An ISO 19115-1 code to indicate the source of the data (image, thematicClassification, physicalMeasurement, auxiliaryInformation, qualityInformation, referenceInformation, modelResult, or coordinate).",
        nullable=False,
        )
    ...
    subject = mapped_column(
        ...
        comment = "A topic of the resource, typically keywords, key phrases or classification codes",
        nullable=True,
        )
    description = mapped_column(
        String(),
        default = "Example: 10 minute average data from romoWind's iSpin sonic anemometer installed on the hub of a wind turbine at Testfeld BHV, Bremerhaven for Fraunhofer IWES within the framework of Testfeld project, 24.07.2019 to 24.07.2022",
        comment = "description of the file answering 5W's (Who, What, Where, Why, When)",
        nullable=True,
        )
    institution = mapped_column(
        String(),
    ...
    references = mapped_column(
        String(), 
        default = None,
        comment = "Published or web-based references that describe the data or methods used to produce it",
        nullable=True,
        )
    source = mapped_column(
        Enum(DataSourceEnum), 
        default = "surface",
        comment = "The method of production of the original data. If it was model-generated, source should name the model and its version, as specifically as could be useful. If it is observational, source should characterize it (e.g., surface observation or radiosonde)",
        nullable=False,
        )
    history = mapped_column(
        String(), 
        default = None,
        comment = "Provides an audit trail for modifications to the original data. Well-behaved generic netCDF filters will automatically append their name and the parameters with which they were invoked to the global history attribute of an input netCDF file. We recommend that each line begin with a timestamp indicating the date and time of day that the program was executed",
        nullable=True,
        )
    ...
    contributor_name = mapped_column(
        Enum(peopleEnum), 
        default = None,
        comment = "The name of any individuals, projects, or institutions that contributed to the creation of this data. May be presented as free text, or in a structured format compatible with conversion to ncML (e.g., insensitive to changes in whitespace, including end-of-line characters)",
        nullable=True,
        )
    contributor_role = mapped_column(
        String(), 
        default = None,
        comment = "The role of any individuals, projects, or institutions that contributed to the creation of this data. May be presented as free text, or in a structured format compatible with conversion to ncML (e.g., insensitive to changes in whitespace, including end-of-line characters). Multiple roles should be presented in the same order and number as the names in contributor_names",
        nullable=True,
        )
    type = mapped_column(
        String(), 
    ...
    users = relationship("User", lazy='select', primaryjoin=creator_name==User.name)
...
class DataFile(Base):
    ...
    date_created = mapped_column(
        DateTime(), 
        default = pd.to_datetime(pa.now(), utc=True),
        comment = "date and time of file creation",
        onupdate=pd.to_datetime(pa.now(), utc=True),
        nullable=True,
    )
...
class Site(Base):
    ...
    platform_code = mapped_column(
        String(), 
        default = None,
        comment = "Reference Id for the platform",
        nullable=True,
    )
    site_address = mapped_column(
        String(), 
        default = "Am Luneort 15, Flughafen Bremerhaven, 27572 Bremerhaven, Germany",
    ...
    geospatial_bounds_crs = mapped_column(
        ...
        comment = "The coordinate reference system (CRS) of the point coordinates in the geospatial_bounds attribute (EPSG CRSs are strongly recommended)",
        nullable=True,
    )
    geospatial_bounds_vertical_crs = mapped_column(
        String(), 
        default = "EPSG:3855",
        comment = "The vertical coordinate reference system (CRS) for the Z axis of the point coordinates in the geospatial_bounds attribute",
        nullable=True,
    )
...
class Documentation(Base):
    ...
    logbook = mapped_column(
        ...
        default = None,
        nullable=True,
        comment = "Name of the tool-list filename",
    )
...
class InstrumentSpecs(Base):
    ...
    category = mapped_column(
        ...
        default = None,
        nullable=True,
        comment = "Name of the instruction manual",
    )
...
class DataAccessLayer:
    ...
    def __init__(self, memory=True):
        ...
        self.session = None
        if not memory:
            self.conn_string = r"sqlite+pysqlite:///C:\Users\giyash\OneDrive - Fraunhofer\Python\Scripts\Metadata\notebooks\metadata_gen.db"
        else:
