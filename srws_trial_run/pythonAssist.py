...
def signal_outliers(df, win_length, sigma, detrend):
...
def window_op(x, method='std'):
    """perform operations on a rolling window (currently only nanstd)"""
    if method=='std':
...
def signal_outliers_vectorized(df, win_length, sigma):
    # find the outliers in the signal following a moving average of window length win
    from scipy import signal
    import pandas as pd
    import numpy as np
...
def multiple_stats_on_df(df, circular_data_cols, stats='mean', degrees=False, circ_pi2pi=[]):
    """
    ...
    Input:
        ...
        kwargs['nondefault_bounds] - list containing True/False for using the default bounds (False) of [0, 2*pi], state True for [-pi, pi] bounds
    ...
    if degrees == False:
        ...
        circ_mean_df = df.filter(circular_data_cols).apply(np.deg2rad).apply(func_circ, axis=0)
        circ_mean_df_nondef = df.filter(circ_pi2pi).apply(np.deg2rad).apply(func_circ, axis=0, high=np.pi, low=-np.pi)
